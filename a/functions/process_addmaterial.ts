process_AddMaterial(
  ...
  try {
    ...
    const getMatId = (color: string): string => {
      const mapping = color === 'NoEdge'
        ? GlobalFunc.find_MaterialMapping('NoEdgeband')
        : GlobalFunc.find_MaterialMapping(color);

      if (!mapping || !mapping.MaterialId) {
