	...
	// Cycle through the starting segments. Starting segments are those where individual segments of the baseboards starts (disregarding the splits).
	const startingSegments = segments.filter(segments => segments.getNeighbourBySide(CKind.Left) === null);
	for (const firstSegment of startingSegments) {
		...
		const plintAreaHeightSpace = firstSegment.generationContour.position.z + firstSegment.generationContour.height;
		/** Space for the legs */
		...
		let lastSegment: LongPartSegmentTypeAlias = firstSegment;
		...
		*/
		...
		if (hasLeftToekick) {
			...
			const leftToekickStartPoint = getPointAtXY(leftToekickPosX, baseboardDepth + leftToekickPosY);
		...
		if (hasRightToekick) {
			...
			const rightToekickEndPoint = getPointAtXY(rightToekickPosX, rightToekickPosY + baseboardDepth);
		...
		if (hasFrontToekick) {
			...
			for (let i = 1; i < toekickSegmentGroups.length; i++) {
				...
				const startSegment = segments[toekickSegmentStartIndex];
