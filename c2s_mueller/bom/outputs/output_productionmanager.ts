...
ol.forEach(p =>
{
  ...
  outStr += '<param name="ArticleDescription" value="' + 'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA' + '" />' + '\n';
...
//===================================================
  ...
  const findBomParent = (parent:string): Map<string, BomOutputFileEntry> =>  {
    ...
    bom.forEach((value, key) => {
      const k = JSON.parse(value.content);
      if (k.entry.parent === parent) {
        r.set(key, value);
      }
    })
    return r;   
  ...
  function insertStringXML(k:any){
    if (k.entry.Type === "Partgroup" && (k.entry.category === target || target == 'TARGET-PRODUCTION-SITE' || target == undefined)){
      ...
      outStr += '<param name="ArticleNumber" value="' + k.entry.Name + '" />' + '\n';
      outStr += '<param name="ArticleDescription" value="' + k.entry.Name + '" />' + '\n';
      //outStr += '<param name="ArticleGroup" value="' + k.entry.parent + '" />' + '\n';
      outStr += '<param name="Quantity" value="1.00000" />' + '\n';
      outStr += '<param name="QuantityUnit" value="pcs" />' + '\n';            
      //outStr += '<param name="Barcode" value="' + o.orderNo + "_" + p.posOrderLineNo+ "_" + k.barcode + '" />' + '\n';
      outStr += '<param name="Length" value="' + k.entry.width + '" />' + '\n';
      outStr += '<param name="Width" value="' + k.entry.depth + '" />' + '\n';
    ...
    if (k.entry.Type === "Board" && (k.entry.category === target || target == 'TARGET-PRODUCTION-SITE' || target == undefined)) {
      outStr += '<entity>' + '\n';
      outStr += '<properties>' + '\n'; 
      outStr += '<param name="Elementtype" value="' + k.entry.Type + '" />' + '\n';            
      ...
      outStr += '<param name="QuantityUnit" value="pcs" />' + '\n';
      //outStr += '<param name="Barcode" value="' + o.orderNo + "_" + p.posOrderLineNo + "_" + k.barcode + '" />' + '\n';
      if(result.has("Images/" + o.posOrderNo + "/" +  p.posOrderLineNo + "_" + k.entry.barcode + '.mpr')){
        outStr += '<param name="CncProgramName" value="Images\\' + o.posOrderNo + "\\" + p.posOrderLineNo+ "_" + k.entry.barcode + '.mpr" />' + '\n';
      }
      
      outStr += '<param name="Material" value="' + k.entry.material + '" />' + '\n';
      outStr += '<param name="Grain" value="' + k.entry.grain + '" />' + '\n';
      outStr += '<param name="GrainTC" value="' + k.entry.grain + '" />' + '\n';
      outStr += '<param name="Length" value="' + k.entry.cutLength + '" />' + '\n';
