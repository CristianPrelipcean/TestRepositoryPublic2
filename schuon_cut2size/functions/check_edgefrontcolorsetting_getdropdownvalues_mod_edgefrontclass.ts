check_EdgeFrontColorSetting_getDropDownValues_mod_EdgeFrontClass(attr: Checks.IEdgeFrontColorSetting_Attributes, selections: SelectionEntry_mod_EdgeFrontClass[]): Checks.CheckDropDownEntry[]{
  if (GlobalFunc.allPropertiesDefined(attr)) {
    const matches = ct_ctab_EdgeColorSettings.filter(x => x.in_BoardColor === attr.mod_Color && x.EdgeColor === attr.mod_EdgeFrontColor);
