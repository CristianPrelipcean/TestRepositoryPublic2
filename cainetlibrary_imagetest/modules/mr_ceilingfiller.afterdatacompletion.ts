  ...
  const addCeilingFiller = (ceilingFillerConstruction: string, ceilingFillerHeight: number) => {
    ...
    ceilingFiller.mod_CeilingFillerHeight = ceilingFillerHeight;
  ...
  startingSegments.forEach(start => {
    ...
    let ceilingFillerHeight = -1;
    let ceilingFillerConstruction = na;
    let seg: LongPartSegmentTypeAlias | undefined = start;
    while (ceilingFillerConstruction === na && seg) {
      ceilingFillerConstruction = seg.getAttributeOrDefault('mod_CeilingFillerConstruction', na);
      ceilingFillerHeight = ceilingFillerHeight > 0 ? ceilingFillerHeight : seg.getAttributeOrDefault('mod_CeilingFillerHeight', -1);
      seg = seg.neighbours.get(CKind.Right)?.other ?? undefined;
    }
    if (ceilingFillerConstruction === na || ceilingFillerHeight <= 0) {
      logError(`Invalid ceiling filler construction or height for segment. Construction: ${ceilingFillerConstruction}, Height: ${ceilingFillerHeight}. The contour: ${JSON.stringify(start.generationContour.contour)}`);
      return;
    }
