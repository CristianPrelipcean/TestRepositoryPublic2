  ...
  if (config) {
    ...
    try {
      ...
      // Add ProcessingItem from table tab_FrontPanelConstruction
      if (retFrontConstruction.retSpecificConstruction.ProcessingItem! != '') {
        let componentList: string[] = [];

        let processingItems = GlobalFunc.find_ProcessingMapping(retFrontConstruction.retSpecificConstruction.ProcessingItem);
        processingItems.forEach((processingItem) => {
          if (processingItem.ProcessingLibrary == "Component") { 
            componentList.push(processingItem.ProcessingId!);
          }
        });
        let processingComponent = JSON.stringify(componentList);
        element.pa_ProcessingComponent = processingComponent;
      }
