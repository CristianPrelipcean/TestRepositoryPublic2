process_BoardBom(Elem: any, part: any ) {
  ...
  try {
    ...
    const bondingOversize = {
      length: part.g?.basic_BondingOversizeLength ?? 20,
      width: part.g?.basic_BondingOversizeWidth ?? 20,
    };
    ...
    if (boardPlan.length === 0) {
      logError("process_BoardBom: BOM could not be created. No plan to build the plate found!");
      return;
    }
