process_BondingBoards(input: {
  ...
  cutWidth: number;
    ...
    if (rest <= 0) {
        logError("Bonding failed: master thickness >= target.");
        return [];
    }
    ...
    // If everything fails
    logError("Bonding algorithm failed. No valid layer combination found.");
    return [];
    }
  ...
  if (matches.length === 0) {
    logError(`No range match found although bonding=false (targetThickness='${target}')`);
    return [];
  }
