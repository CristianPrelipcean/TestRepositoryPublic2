  ...
  //
  // Info: Adjusted script to match the requirements for a successful data conversion and mapping using the homag connect converter.
  // Modifications to original:
  // - Fixed typo (schemLocatition to schemaLocation) => NEEDED in original script
  // - Added external system id to order line entries => NEEDED in original script
  // - Adjusted quantity from double to int value => NOT NEEDED in original script, will be fixed in converter to be more tolerant
  //
  // Description:
  ...
  outStr += '<param name="AddressField1" value="Herr" />' + '\n';
  ...
  ol.forEach(p => {
    ...
    //outStr += '<param name="Barcode" value="' + o.orderNo + '_' + countOL + '" />' + '\n';
    outStr += '<param name="OrderItemNumber" value="' + countOL + '" />' + '\n';
    outStr += '<param name="ArticleNumber" value="' + p.posArticleNo + '" />' + '\n';
    outStr += '<param name="ArticleDescription" value="' + p.posArticleNo + '" />' + '\n';
    outStr += '<param name="ArticleGroup" value="' + p.posOrderLineNo + '" />' + '\n';
    ...
    function insertStringXML(k: any) {
      if (k.Type === "Partgroup" && (k.category === target || target == 'TARGET-PRODUCTION-SITE' || target == undefined)) {
        ...
        outStr += '<param name="Quantity" value="1" />' + '\n';
        outStr += '<param name="QuantityUnit" value="pcs" />' + '\n';
        //outStr += '<param name="Barcode" value="' + o.orderNo + "_" + p.orderLineNo + "_" + k.barcode + '" />' + '\n';
