process_EdgeInfo(PartID:string, PartThickness:number, ColorFront:string, ColorLeft:string, ColorBack:string, ColorRight:string, EdgeFrontType:string, EdgeLeftType:string, EdgeBackType:string, EdgeRightType:string, EdgeJointType:string, ) {
/*
//====================================================================
...
//---------------Get EdgeData-----------------
let EdgeFrontData = getEdgeData(EdgeFrontType,	ColorFront,	PartThickness);
let EdgeLeftData = 	getEdgeData(EdgeLeftType,	ColorLeft,	PartThickness);
let EdgeBackData = 	getEdgeData(EdgeBackType,	ColorBack,	PartThickness);
let EdgeRightData = getEdgeData(EdgeRightType,	ColorRight,	PartThickness);
...
EdgeShape				: EdgeShape,
OverdimensionFront :	EdgeFrontData	?  getPartOverdimension(EdgeFrontType, EdgeFrontData.in_Thickness!)	: 0,
OverdimensionLeft :	EdgeLeftData	?  getPartOverdimension(EdgeLeftType, EdgeLeftData.in_Thickness!) 		: 0,
OverdimensionBack :	EdgeBackData	?  getPartOverdimension(EdgeBackType, EdgeBackData.in_Thickness!) 		: 0,
OverdimensionRight :	EdgeRightData	?  getPartOverdimension(EdgeRightType, EdgeRightData.in_Thickness!)	: 0,
}
...
function getEdgeJoint (partID: string, edgeCode: string, edgeNeighbourCode: string, edgeType: string, edge:string): string {
	...
	else if (edgeCode != 'NoEdge' && edgeNeighbourCode == 'NoEdge')
	{
 	...
 	let partSettings = GlobalFunc.find_PartSettings(partID);
	let e = GlobalFunc.find_EdgeJoint(partSettings.PartGroup!,edgeType)!;
...
function getEdgeData (edgeType: string, color: string, partthickness: number): ICT_tab_EdgeLibrary|undefined {
	if (edgeType != 'NOE') 
	{
		let edgeSettings = GlobalFunc.find_EdgeSettings(edgeType);
		let edgeMapping = GlobalFunc.find_EdgeMapping(edgeSettings.EdgeClass!,color);
		if (edgeMapping.DecorCode! == 'NoEdge')
			{
				return undefined;
			}
		else
			{
				let edgeLibrary = GlobalFunc.find_EdgeLibrary(edgeMapping.DecorCode!,edgeMapping.Thickness!,partthickness);
				return edgeLibrary;
			}
	}
	return undefined;
...
function getPartOverdimension (edgeType: string, edgethickness: number): number {
	if (edgeType != 'NOE' && GlobalFunc.find_PartOverdimension(edgethickness)!) 
	{
		return GlobalFunc.find_PartOverdimension(edgethickness).Overdimension;
	}
	return 0;
};
